...
import { TIME_SLOTS, DAYS } from '@/lib/timetable'
...
export default function WeeklyTimetableView({ 
  ...
  // Create a map of day -> time_slot -> entry
  const scheduleMap = new Map<string, Map<string, TimetableEntry>>()
  ...
  entries.forEach(entry => {
    ...
    if (dayMap) {
      dayMap.set(entry.Time_Slot, entry)
    }
  })
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className={`bg-gradient-to-r ${
        type === 'room' ? 'from-blue-600 to-cyan-600' : 'from-orange-600 to-red-600'
        ...
        <div className={`flex items-center gap-6 ${type === 'room' ? 'text-blue-100' : 'text-orange-100'} text-sm`}>
          <div className="flex items-center gap-2">
            ...
            <span>{occupiedSlots} {type === 'room' ? 'bookings' : 'classes'} scheduled</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {/* Weekly Timetable Grid - TRANSPOSED (Days as Rows, Time as Columns) */}
            ...
            <thead>
              <tr className="bg-gradient-to-r from-gray-50 to-gray-100">
                ...
                {TIME_SLOTS.map((slot, index) => (
                  <th 
                    key={slot} 
                    className={`px-3 py-4 text-center border-l border-gray-200 min-w-[160px] ${
                      highlightSlot === slot ? 'bg-orange-100' : ''
                    }`}
                  >
                    <div className={`font-bold text-sm ${
                      highlightSlot === slot ? 'text-orange-600' : 'text-gray-800'
                    }`}>
                      {slot}
                    </div>
                ...
                return (
                  <tr 
                    ...
                    className={`border-t border-gray-200 ${
                      isHighlightedDay ? 'bg-orange-50' : ''
                    }`}
                  >
                    {/* Day Column */}
                    <td className={`sticky left-0 z-10 px-4 py-4 border-r-2 border-gray-300 font-semibold ${
                      isHighlightedDay ? 'bg-orange-100' : 'bg-white'
                    }`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500">
                            {dayMap ? Array.from(dayMap.values()).length : 0} {type === 'room' ? 'bookings' : 'classes'}
                          </div>
                        </div>
                      </div>
                    </td>
                          ...
                          {isEmpty ? (
                            ...
                            <div className="space-y-2">
                              ...
                              <div className="flex items-start gap-2">
                                <div className="w-8 h-8 rounded-md bg-gradient-to-br from-orange-400 to-pink-400 flex items-center justify-center flex-shrink-0">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-center">
          <div className="text-3xl font-bold text-orange-600">{occupiedSlots}</div>
          <div className="text-sm text-orange-700 mt-1">{type === 'room' ? 'Bookings' : 'Classes'}</div>
        </div>
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
        <div className="flex items-center justify-center gap-8 text-sm flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-700">{type === 'room' ? 'Occupied' : 'Scheduled Class'}</span>
          </div>
          <div className="flex items-center gap-2">
